 ...
 * This demo detects 6d orientation using a IKS01A3 shield.
 */
...
static const LSM6DSOConfig lsm6dsocfg = {
  ...
  .gyrooutdatarate    = LSM6DSO_GYRO_ODR_26Hz,
  .lirvalue           = LSM6DSO_ALL_INT_LATCHED,
  .sixdthreshold      = LSM6DSO_DEG_60,
  .sixdlowpassenable  = 1U,
  .sixd4dmodeenable   = 0U,
};
...
int main(void) {
  ...
  /*
   * Loop that checks if 6d orientation has changed and prints
   * the new orientation to the terminal.
   */
  while (true) {
        ...
        if (all_source.six_d) {
          chprintf(chp, "6D orientation switched to: ");
