 ...
 * This demo counts steps using a IKS01A3 shield.
 */
...
static const LSM6DSOConfig lsm6dsocfg = {
  ...
  .pedomode           = LSM6DSO_FALSE_STEP_REJ,
};
...
int main(void) {
  ...
  /*
   * Loop that gets the step count and prints it to the terminal.
   */
  while (true) {
      uint16_t val;
      lsm6dsoNumberOfStepsGet(&LSM6DSOD1, &val);

        chprintf(chp, "number of steps: %d\r\n",val);
        chThdSleepMilliseconds(500);
      }
}
