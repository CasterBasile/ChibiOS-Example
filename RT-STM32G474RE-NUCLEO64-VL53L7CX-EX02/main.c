...
/*
 * [NISC2025-VL53L7CX-EX02] - VL53L7CX demo 3.
 * This demo shows how to communicate data to a python script.
 * It can be useful to train machine learning models and visualize data.
 */
...
int main(void) {
  ...
  while (1) {
    ...
    if (isReady) {
      ...
      // Dump 64 distances in mm
      for (i = 0; i < 64; i++) {
        uint16_t dist = Results.distance_mm[VL53L7CX_NB_TARGET_PER_ZONE * i];
        chprintf(chp, "%u", dist);
        if (i < 63) {
          chprintf(chp, ",");
        }
